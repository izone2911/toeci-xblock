    ...
    template(v-for="(q, index) in questions" :key="q.id")
      ...
      .accordion-item(
        ...
        .accordion-body(v-show="expandedIds.includes(q.id)")
          ...
          // ==========================================
          // NỘI DUNG CÂU HỎI VÀ ĐÁP ÁN (CÂU ĐƠN LẺ)
          // ==========================================
          .question-content-box
            
            // KHỐI GÓI GỌN CÂU HỎI VÀ GIẢI THÍCH
            .shared-context-box.mb-4
              .top-row-grid
                ...
                .editor-card.q-content-card
                  ...
                  RichTextEditor(
                    v-model="q.content" 
                    placeholder="Ví dụ: Mr. Smith is currently ________ a meeting in Tokyo."
                  )

                // CỘT PHẢI: GIẢI THÍCH / TRANSCRIPT
                .editor-card.explanation-card
                  h4 Giải thích / Transcript
                  RichTextEditor(
                    v-model="q.explanation" 
                    placeholder="Nhập giải thích chi tiết hoặc transcript cho câu này..."
                  )
      ...
      // ĐƯỜNG KẺ PHÂN CÁCH NÉT LIỀN
      .group-divider(v-if="index !== questions.length - 1")
        .divider-line

  .empty-state(v-else)
    h4 Chưa có câu hỏi nào
    button.btn-add-large.mt-4(@click="addNewQuestion") + Bắt đầu thêm câu
...
const onDropEditor = (targetIndex: number) => {
  if (dragEditorIndex.value !== null && dragEditorIndex.value !== targetIndex) {
    ...
    openModal(`Bạn muốn đổi vị trí giữa hai câu này?`, 'swap', () => {
...
const createNewObject = () => ({
  ...
  content: '', 
  explanation: '', // Đã bổ sung trường giải thích
  options: ['', '', '', ''],
  correctAnswer: ''
});
...
<style scoped>
/* =========================================================
   BỘ CSS CHUẨN ĐƯỢC ĐỒNG BỘ 100% TỪ PART 1 CỦA BẠN
   ========================================================= */
...
/* KHỐI GÓI GỌN CÂU HỎI VÀ GIẢI THÍCH MỚI */
.shared-context-box { background: #e0f2fe; padding: 20px; border-radius: 12px; border: 1px solid #bfdbfe; }
...
.options-card h4, .q-content-card h4, .explanation-card h4 { margin: 0 0 16px 0; color: #334155; font-size: 1rem; font-weight: 600; }
...
@media (max-width: 1100px) { .top-row-grid, .options-grid { grid-template-columns: 1fr; gap: 16px; } }
</style>