...
.part-page
    ...
    template(v-for="(q, index) in questions" :key="q.id")
      ...
      .accordion-item(
        ...
        .accordion-body(v-show="expandedIds.includes(q.id)")
          ...
          // ==========================================
          // ĐOẠN THÔNG TIN (CHỈ HIỂN THỊ Ở CÂU đầu tiên mỗi nhóm 3 câu)
          // ==========================================
          .shared-context-box(v-if="index % 3 === 0")
            h3.shared-title Bài nói ngắn
            .top-row-grid
              // HÌNH ẢNH
              ...
              .editor-card.transcript-card
                h4 Transcript
                RichTextEditor(v-model="q.content" placeholder="Nhập transcript của bài nói vào đây...")
  ...
  // Nút thêm một cụm bài nói mới (3 câu hỏi)
