    ...
    template(v-for="(q, index) in questions" :key="q.id")
      ...
      .accordion-item(
        ...
        .accordion-header(
          ...
          title="Bấm để mở rộng/thu gọn, kéo thả để hoán đổi"
        )
          .header-left
            span.drag-handle ⣿
            span.q-title {{ startNumber + index }}
            
            // VÒNG TRÒN HIỂN THỊ ĐÁP ÁN (Chỉ A, B, C)
        ...
        // NỘI DUNG CHỈNH SỬA (Layout 2 cột)
        .accordion-body(v-show="expandedIds.includes(q.id)")
          .top-row-grid
            
            // CỘT TRÁI: LỜI THOẠI CÂU HỎI
            .editor-card.transcript-card
              h4 Transcript
              RichTextEditor(v-model="q.content" placeholder="Nhập transcript của người hỏi...")

            // CỘT PHẢI: 3 ĐÁP ÁN A, B, C
...
const createNewObject = () => ({
  ...
  content: '',
  options: ['', '', ''], // Part 2 CHỈ CÓ 3 ĐÁP ÁN (A, B, C)
  correctAnswer: ''
});
...
.transcript-card h4, .options-card h4 { margin: 0 0 16px 0; color: #334155; font-size: 1rem; font-weight: 600; }
...
@media (max-width: 900px) { .top-row-grid { grid-template-columns: 1fr; gap: 16px; } }
</style>