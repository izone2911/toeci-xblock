...
.landing-wrapper
  ...
  .modal-overlay(v-if="showBlockedModal")
    .custom-modal
      ...
      p.modal-message Bạn đã hoàn thành bài làm. Bài tập này không cho phép xem lại kết quả chi tiết.
      .modal-actions
...
// 🔥 ĐÃ TỐI ƯU: Hiển thị chính xác thời gian kể cả khi có số giây lẻ
...
const isVocabLocked = computed(() => {
  // Đã chưa bắt đầu, hoặc đã nộp bài xong -> Mở khóa
  if (!store.hasStarted || store.isSubmitted) return false;
  
  // 🔥 ĐÃ TỐI ƯU: Cứ đang làm bài là khóa, không phân biệt tính giờ hay không tính giờ
  return true; 
});
...
<style scoped>
/* FIX MÔI TRƯỜNG EDX: Dùng min-height thay cho 100vh để chống bóp méo */
