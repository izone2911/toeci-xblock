  ...
  .questions-list(v-if="groupedQuestions && groupedQuestions.length > 0")
    ...
    .editor-card(v-for="(group, gIdx) in groupedQuestions" :key="'group_' + gIdx")
      .group-body(:class="{ 'has-graphic': group.imageUrl }")
        ...
        // CỘT ẢNH MINH HỌA (Tự động hiển thị nếu cụm câu hỏi có hình ảnh)
        .image-col(v-if="group.imageUrl")
          img.q-img(:src="group.imageUrl" alt="Part 3 Graphic")
        
        // CỘT DANH SÁCH 3 CÂU HỎI
        .questions-col
          // Từng câu hỏi nay đã có border bo góc rõ ràng và KHÔNG CÓ viền xanh khi focus
          .sub-question-block(
      ...
      .transcript-zone(v-if="store.isSubmitted && group.transcript")
        .transcript-header 📖 Lời thoại (Transcript):
        .transcript-text {{ group.transcript }}

  .empty-state(v-else)
    .empty-icon 👥
    h4 Chưa có dữ liệu câu hỏi cho Part 3
</template>
...
// Thuật toán tự động quét và gom mảng phẳng thành từng cụm 3 câu hỏi chung một nhóm
const groupedQuestions = computed(() => {
  ...
  for (let i = 0; i < rawData.length; i += 3) {
    ...
    // Tự động tìm xem trong 3 câu này có câu nào chứa ảnh hoặc đoạn văn bản transcript không
...
.sub-question-block { 
  padding: 16px; 
  border-radius: 8px; 
  border: 1px solid #cbd5e1; /* Thêm border giống hệt Part 1, 2 */
...
/* TRẠNG THÁI MÀU SẮC ĐÁP ÁN (XANH/ĐỎ) */
